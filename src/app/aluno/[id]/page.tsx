...
type Falta = { data: string; presente: boolean }
type Nota = { disciplina: string; valor: string; descricao: string; bimestre: number }
...
export default function AlunoPerfil() {
  ...
  async function carregar() {
    ...
    const { data: e } = await supabase.from("escolas").select("nome").eq("id", a.escola_id).single()
    if (e) setEscolaNome(e.nome)

    const { data: f } = await supabase.from("faltas").select("data, presente").eq("aluno_id", id).order("data", { ascending: false })
    if (f) setFaltas(f)

    const { data: n } = await supabase.from("notas").select("*").eq("aluno_id", id).order("bimestre").order("disciplina")
    if (n) setNotas(n)
  }
  ...
  if (!aluno) return <div className="card p-12 text-center text-slate-400">Carregando...</div>
